...
for i, [x, name, y] in enumerate(data_loader):
    ...
    for k in range(num_iter):
        ...
        gt = momentum * gt + (new_grad + vt) / (new_grad + vt).norm(1)
        ...
        for m in range(sample_number):
            ...
            output2 = s_model(x_neighbor)
            loss = torch.nn.functional.cross_entropy(output2[0], y)
            loss += torch.nn.functional.cross_entropy(output2[1], y) * 0.4
            loss.backward()
            global_grad += x_neighbor.grad
        vt = global_grad / (1. * sample_number) - new_grad
